...
function _measurePick(event) {
  ...
  if (mode === 'pp') {
    if(!_measureP1) {
      _measureP1 = pt; _measureSphere(pt, 0xff4444);
      $('msr-hint').textContent = 'P1 gesetzt — Klick: P2 setzen';
    } else {
      _measureP2 = pt; _measureSphere(pt, 0x44ff88);
  ...
  // 3-Punkt-Kreis
  if (typeof _circle3pts === 'undefined') return;
...
function _snapToVertex(hitPoint) {
...
// ── Rechenoperationen in Positionsfeldern ──────────────────────────
(function() {
  function evalField(input) {
    const raw = input.value.trim();
    if (!raw) return;
    try {
      // Nur einfache Mathematik erlauben: Zahlen, +, -, *, /, (, ), Leerzeichen, Punkt, Komma
      ...
      input.style.background = 'rgba(200,50,50,.2)';
    }
  }

  // Alle Positionsfelder erfassen (type=number und type=text)
  const posSelectors = [
    'fm-x','fm-y','fm-z','fm-rx','fm-ry','fm-rz',
    'pm-ox','pm-oy','pm-oz','pm-orx','pm-ory','pm-orz',
    'om-ox','om-oy','om-oz','om-orx','om-ory','om-orz',
  ];

  function attachCalc(id) {
    const el = document.getElementById(id);
    if (!el) return;
    // type=text damit Ausdrücke eingegeben werden können
    el.type = 'text';
    el.addEventListener('blur', () => evalField(el));
    el.addEventListener('keydown', e => { if (e.key === 'Enter') { evalField(el); e.target.blur(); } });
  }

  // Nach DOM-ready
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => posSelectors.forEach(attachCalc));
  } else {
    posSelectors.forEach(attachCalc);
  }

  // Auch dynamisch erstellte Felder in der Parametertabelle (renderRows)
  window._attachCalcToField = attachCalc;
})();
