...
async function _collectEntries(entry, out) {
  if (entry.isFile) {
    ...
    await new Promise(res => reader.readEntries(async entries => {
      for (const en of entries) await _collectEntries(en, out);
      res();
    }));
  }
}
...
function _dropNextStl() {
  ...
  if (/umfeld|umgebung|environment|fence|zaun/i.test(nm)) guess = 'Umgebung';
...
function _showDropAxisPicker(file, preselect) {
  ...
  if (!dlg) {
    ...
    dlg.style.cssText = 'position:fixed;inset:0;z-index:9500;background:rgba(0,0,0,.65);display:flex;align-items:center;justify-content:center';
    document.body.appendChild(dlg);
  }
  var remaining = _dropPendingStls.length;
  dlg.innerHTML = '<div style="background:var(--bg2);border:1px solid var(--acc);border-radius:8px;padding:18px 22px;font-family:monospace;min-width:320px;max-width:420px">'
    +'<div style="color:var(--acc);font-weight:700;margin-bottom:4px;font-size:.9em">📂 STL zuweisen ('+(remaining)+' verbleibend)</div>'
    +'<div style="color:var(--txt2);font-size:.82em;margin-bottom:12px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" title="'+file.name+'">'+file.name+'</div>'
  ...
  var cats = [
    {label:'A1',color:''},     {label:'A2',color:''},   {label:'A3',color:''},
    {label:'A4',color:''},     {label:'A5',color:''},   {label:'A6',color:''},
    {label:'Podest',color:'#334455'}, {label:'Tool',color:'#2563eb'},
    {label:'Endeffektor',color:'#607080'}, {label:'Umgebung',color:'#4a6040'},
    {label:'Positionierer',color:'#605040'}, {label:'Schiene',color:'#404060'},
    {label:'Festes Obj.',color:'#606060'}, {label:'Bew. Obj.',color:'#404050'},
  ];
  cats.forEach(function(cat) {
    var b = document.createElement('button');
    b.textContent = cat.label;
    var isSelected = cat.label===preselect;
    b.style.cssText = 'padding:4px 6px;border-radius:3px;cursor:pointer;font-family:monospace;font-size:.78em;white-space:nowrap;'
      +(isSelected?'background:var(--acc);color:#000;border:none;font-weight:700;':'background:var(--bg3);border:1px solid var(--bdr);color:var(--txt2);');
    b.addEventListener('click', (function(label){ return function(){ _dropAssignStl(file, label); }; })(cat.label));
    btns.appendChild(b);
  });
...
async function _dropAssignStl(file, cat) {
  ...
  try {
    ...
    if (/^A[1-6]$/.test(cat)) {
      ...
      if (!state.axisStlParts[cat].find(p=>norm(p.name)===norm(displayName)))
        state.axisStlParts[cat].push({name:displayName,color:'#e8a020',buf:stlBuf});
      state.axisStlMap[cat]=state.axisStlParts[cat][0]?.name||displayName;
      state.buffers.set(displayName,stlBuf);
      if (!state.stls.find(f=>f.name===displayName)) state.stls.push({path:displayName,name:displayName,type:'STL',size:stlBuf.byteLength});
      state.files=state.stls;
      const mesh=new THREE.Mesh(geom,new THREE.MeshStandardMaterial({color:'#e8a020',roughness:.62,metalness:.08}));
      mesh.name=displayName; meshes.set(displayName,mesh);
      ...
      if (!state.stls.find(f=>f.name===displayName)) state.stls.push({path:displayName,name:displayName,type:'STL',size:stlBuf.byteLength});
      state.files=state.stls;
      const mesh=new THREE.Mesh(geom,new THREE.MeshStandardMaterial({color:cat==='Podest'?'#334455':'#2563eb',roughness:.6,metalness:.1}));
      mesh.name=displayName; meshes.set(displayName,mesh);
      rebuildRobotKinematics(); applyTransforms();

    } else if (cat==='Endeffektor') {
      const eff = { name: displayName.replace('.stl',''), objectType:'stl', color:'#607080',
        offset:{x:0,y:0,z:0,rx:0,ry:0,rz:0}, ePos:0,
        teile:[{name:displayName,objectType:'stl',color:'#607080',offset:{x:0,y:0,z:0,rx:0,ry:0,rz:0},
          stlFile:{name:displayName,buf:stlBuf}}] };
      state.effektoren.push(eff); effektorGroups.push(null);
      state.activeEff=state.effektoren.length-1;
      rebuildEffMesh(state.activeEff); renderEffRow?.();

    } else if (cat==='Umgebung') {
      state.umfElemente.push({ name:displayName.replace('.stl',''), stlFile:{name:displayName,buf:stlBuf,path:displayName},
        offset:{x:0,y:0,z:0,rx:0,ry:0,rz:0} });
      state.buffers.set(displayName,stlBuf);
      rebuildAllUmf?.(); renderUmfRow?.(); renderAll();

    } else if (cat==='Positionierer') {
      state.positioners = state.positioners||[];
      const pos = { name:displayName.replace('.stl',''), joints:[],
        stlFile:{name:displayName,buf:stlBuf}, offset:{x:0,y:0,z:0,rx:0,ry:0,rz:0} };
      state.positioners.push(pos); positionerGroups.push(null);
      rebuildPositionerMesh?.(state.positioners.length-1); renderAll();

    } else if (cat==='Schiene') {
      state.schienen = state.schienen||[];
      state.schienen.push({ name:displayName.replace('.stl',''), stlFile:{name:displayName,buf:stlBuf},
        offset:{x:0,y:0,z:0,rx:0,ry:0,rz:0} });
      rebuildAllSchienen?.(); renderAll();

    } else if (cat==='Festes Obj.') {
      state.festeObjekte = state.festeObjekte||[];
      state.festeObjekte.push({ name:displayName.replace('.stl',''), stlFile:{name:displayName,buf:stlBuf},
        offset:{x:0,y:0,z:0,rx:0,ry:0,rz:0} });
      rebuildAllFeste?.(); renderAll();

    } else if (cat==='Bew. Obj.') {
      state.objekte = state.objekte||[];
      state.objekte.push({ name:displayName.replace('.stl',''), stlFile:{name:displayName,buf:stlBuf},
        offset:{x:0,y:0,z:0,rx:0,ry:0,rz:0}, mountMode:'fixed' });
      rebuildObjektMesh(state.objekte.length-1); renderAll();
    }
